...
from game_ros.srv import GetUserScore
...
class ResultGameNode:
    def __init__(self):
        ...
        self.userscore = rospy.Service("getuserscore", GetUserScore, self.fetchScore)
    ...
    def maybe_print_result(self):
        if self.user_info is not None and self.score is not None:
            ...
            self.scores[str(self.user_info.username)] = int(self.score)
    
    def fetchScore(self, req):
        try:
            if self.scores[req.username]:
                return int(self.scores[req.username])
        except:
            return 0
